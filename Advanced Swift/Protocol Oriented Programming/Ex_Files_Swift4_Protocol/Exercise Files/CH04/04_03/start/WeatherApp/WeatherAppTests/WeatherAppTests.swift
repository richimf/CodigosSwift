...
class WeatherAppTests: XCTestCase {
    ...
    func testOpenWeatherMap() {
        ...
        let controller = OpenMapWeatherNetworkController()
        let city = "Carpinteria"
        controller.self.fetchCurrentWeatherData(for: city) { (weatherData, error) in
            XCTAssertNil(error, "fetchWeatherData() call returned error: \(error?.localizedDescription ?? "")")
            if let data = weatherData {
                print("Weather in \(city): \(data.weather), \(data.temperature)\(data.unit)")
                exp.fulfill()
            } else {
